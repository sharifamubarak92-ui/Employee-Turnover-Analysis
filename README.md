# Employee Data Analysis


**Author:** Sharifa Mubarak | **Date:** 2026

---

## 🔗 Interactive Dashboard (Tableau)

[View the Full Interactive Story Here](https://public.tableau.com/views/hr_17793160776310/Story)

Click the link to explore 9 pages of interactive dashboards.

---

## 📁 Project Files

| File | Description |
|------|-------------|
| `Employee_Analysis.pdf` | **Full Report** (11 pages) - Detailed analysis, findings, and recommendations |
| `EMPLOYEE DATA ANALYSIS.pdf` | **Presentation Slides** - Summary for quick review |
| `TABLEAU_LINK.txt` | Interactive Tableau dashboard link |

---

## 📊 Key Findings

| Finding | Result |
|---------|--------|
| **Data Quality Issue** | 38% of employee records were misclassified |
| **True Turnover Rate** | 51.1% (not 12.9%) |
| **Training Impact** | Failed: 54.3% left / Completed: 48.6% left |
| **Software Engineering** | 60% voluntary turnover |
| **Sales Department** | 37% retirement |

---

## 🛠 Tools Used

- **Python** (Pandas, NumPy) - Data cleaning & analysis
- **Tableau Public** - Interactive dashboards & story
- **Excel** - Initial data exploration

---

## 🚀 Top 5 Recommendations

1. **Improve Training Programs** - Reduce failure rate from 25% to below 15%
2. **Focus on Software Engineering** - Improve engagement and career growth
3. **Succession Planning for Sales** - Knowledge transfer before retirement wave
4. **Conduct Exit Interviews** - Understand why employees leave
5. **Automate Data Quality Checks** - Prevent future errors

---

## 📝 Limitations

This is **synthetic data** from Kaggle, created for educational purposes. Correlations between variables were near zero, confirming its synthetic nature.

---

*Part of the Google Data Analytics Certificate*